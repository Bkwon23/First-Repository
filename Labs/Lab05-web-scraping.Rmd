...
dickinson_poems <- html_table(tables[[1]])
dickinson_poems
...
```{r}
dickinson_poems <- dickinson_poems %>%
  mutate(URL = str_replace_all('First Line (often used as title)', " ", "_")) %>%
  select(URL) %>%
  mutate(URL = paste0("https://en.wikisource.org/wiki/"))

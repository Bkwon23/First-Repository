...
path_in <- "~/Spring 2021/Data Science - STAT 231/First-Repository/Labs/Unemployment.xls"

college_plans0 <- read_csv(paste0(path_in, "/Unemployment.csv"))
...
county_employment <- readxl::read_excel(paste0("~/Spring 2021/Data Science - STAT 231/First-Repository/Labs/Unemployment.xls"), sheet = 1, skip = 7) %>%
  janitor::clean_names()
...
mass_county_df <- final_df %>% 
  filter(region == "massachusetts")

ggplot(mass_county_df, aes(x = long, y = lat, group = group, fill = unemployment_rate_2000)) +
  geom_polygon(colour = "white") +
  theme_void() +
  scale_fill_viridis(option = "magma", direction = -1) +
  coord_fixed(ratio = 1.3)

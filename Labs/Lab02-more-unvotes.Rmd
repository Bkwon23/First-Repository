...
- Try adding `shape = 2` to the geom_point() function.  What happens? 
  ...
  The shape of the points turns into triangles.

- What happens if you add `shape = "*", size = 5`?  
  
  The shape of the points turns into "*" and these points are also bigger in size.
...
```{r, fig.width=10}
g + geom_point() + aes(shape = country, color = country)
```

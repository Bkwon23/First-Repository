...
> ANSWER: I think after discussing the ethics questions of "Predicting Policing & Recidivism," my perspective has still not changed, and I still believe there is bias in terms of using algorithms to detect criminals.  What I found surprising during my discussion was a question someone actually asked the entire group: "Are humans allowed to make more mistakes than an algorithm is allowed to make?"  Therefore, in essence, "Can our own creations be allowed to make more mistakes than us ourselves?"  I still think about this question to this day.
...
> ANSWER: I think after discussing the ethics questions of college admissions, my perspective has changed in terms of using algorithms for the admissions process.  I initially thought that using algorithms for this scenario was unreliable and that it was necessary to use a 100%-human structure to maintain the admissions process.  However, as more students apply, and as college gets even tougher to get into, I understand that using an alogorithm is necessary to lighten the load on humans.
...
km_baseball_std <- kmeans(hof_cluster[,vars_std]
                        , centers=3
                        , nstart=20)
...
km_baseball_unstd <- kmeans(hof_cluster[,vars_unstd]
                        , centers=3
                        , nstart=20)
hof_cluster <- hof_cluster %>%

...
> ANSWER: "head(str_subset(macbeth, "  MACBETH")) displays six lines that include the word "MACBETH."  To which I quote the textbook, the str_subset function works like finding a "needle in a haystack," where the first argument (macbeth) functions like a character vector in which you are looking for patterns and the second argument is the regular expression you need to find (which is "MACBETH" in this case).  We have, therefore, printed out the first six lines in which the computer has found the word "MACBETH."

"head(str_which(macbeth, "  MACBETH)) displays the line numbers in which we have found the first six lines that have included the word "MACBETH" in them.  The function str_which returns the indices of this "haystack" in which the needles were discovered.  As a result, the str_which function displays numbers.  

"head(str_detect(macbeth, "  MACBETH"))" is a logical command that looks for lines that have the string "  MACBETH."  Because the six instances displayed by "head" do not contain this string, they are all labeled false.    
...
> ANSWER: The two backslashes ("\\") are used to search for the literal value of a metacharacter, which in this case, is the period.  Therefore, when these backslashes are used, the period (in essence) is a "part" of the string of characters that are being searched for.  When the backslashes aren't used, as in 'head(str_subset(macbeth, "MACBETH."))', the period is used purely as a metacharacter only.  As a result, as in the case of the second result, for example, "MACBETH" is followed by a comma because the period is not a character being searched for.
...
> ANSWER: In the first command ('head(str_subset(macbeth, "MAC[B-Z]"))'), this pattern will match any lines that contain "MAC" followed by any capital letter besides "A."  In the second command (`head(str_subset(macbeth,"MAC[B|Z]"))`), this pattern will match any lines that contain either "MACB" or "MACZ."  In the third command (`head(str_subset(macbeth, "^MAC[B-Z]"))`), this pattern will match any lines that contain "MAC" followed by any capital letter besides "A" but also make sure to anchor this pattern to the beginning of a piece of text.  In other words, in the beginning of the string, there must occur "MAC" followed by any character from B through Z.
